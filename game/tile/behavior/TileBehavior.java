...
public class TileBehavior {
    ...
    public void onPlace(TileStateWithPos state, Player player) {
        if (state.tile().getSprite() instanceof AnimatedSprite animatedSprite) {
            ...
            animatedSprite.setObserver((oldImage, newImage) -> {
                System.out.println("removeSprite");
                GameEngine.getInstance().forceUpdate(new Rectangle(
                        state.cell().toPixel().x(),
                        state.cell().toPixel().y(),
                        animatedSprite.getWidth(),
                        animatedSprite.getHeight()
                ));
            });
            GameEngine.getInstance().getSchedulerService().getAnimationTask().addSprite(state.world(), animatedSprite);
